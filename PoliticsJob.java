...
public class PoliticsJob
{
	public void executeJob(String[] args) throws IOException, ClassNotFoundException, InterruptedException
	{
		...
		Job job = Job.getInstance(conf, "politics_job");
		job.getConfiguration().set("mapreduce.output.basename", "politics_job");
		...
		FileOutputFormat.setOutputPath(job, new Path(args[53]));
